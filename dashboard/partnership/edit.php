...
if ($id <= 0) {
    ...
    header('Location: /dashboard/partnership');
    exit;
}
...
<div class="flex min-h-screen overflow-hidden">
    ...
    <main class="flex-1 min-w-0 h-screen overflow-y-auto px-4 md:px-8 space-y-4 pb-4">
        ...
        <div class="px-4 md:px-8 py-4">
            <div class="flex items-center justify-between">
                ...
                <a href="/dashboard/partnership"
                    ...
                    <span class="hidden sm:inline">Back to Partnership</span>
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm">
            <form action="/dashboard/partnership/process.php" method="POST" enctype="multipart/form-data" class="space-y-6" id="partnershipForm">
                <script>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" required placeholder="Judul partnership..."
                    ...
                    <div>
                        ...
                        <textarea name="quete" rows="3" placeholder="Quote / teks..."
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all"><?php echo htmlspecialchars($row['quete'] ?? ''); ?></textarea>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">Title List (judul untuk blok list)</label>
                        <input type="text" name="title_list" placeholder="Contoh: Partner Kami"
                            value="<?php echo htmlspecialchars($row['title_list'] ?? ''); ?>"
                            class="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 focus:border-transparent transition-all">
                    </div>
                    <div>
                        ...
                        <div class="mb-2">
                            <img id="imagePreview" src="<?php echo htmlspecialchars((strpos($row['image'], '/') === 0 ? '' : '/') . $row['image']); ?>" alt="" class="h-24 w-auto object-cover rounded-xl border border-slate-200">
                        </div>
                        <?php endif; ?>
                ...
                <div class="border-t border-slate-200 pt-6">
                    <div class="flex items-center justify-between mb-3">
                        <label class="block text-sm font-semibold text-slate-700">List (hanya title)</label>
                ...
                <div class="flex items-center gap-4 pt-4 border-t border-slate-100">
                    <button type="submit" form="partnershipForm" onclick="return validateForm()"
