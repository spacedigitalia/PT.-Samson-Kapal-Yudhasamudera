...
switch ($action) {
    case 'create':
        ...
        $buttonLink = trim($_POST['button_link'] ?? '') ?: null;
        if ($title === '' || $description === '') {
            $_SESSION['error'] = 'Judul dan deskripsi wajib diisi.';
            header('Location: /dashboard/home/create.php');
            exit;
        }
        $image = handleImageUpload();
        if ($image === '') {
            $image = '/assets/logo.jpg'; // fallback
        }
        if ($controller->create($title, $description, $image, $buttonText, $buttonLink, $userId)) {
            $_SESSION['success'] = 'Konten home berhasil ditambahkan.';
        } else {
    ...
    case 'update':
        ...
        if ($title === '' || $description === '') {
            $_SESSION['error'] = 'Judul dan deskripsi wajib diisi.';
            header('Location: /dashboard/home/edit.php?id=' . $id);
            exit;
        }
        ...
        if ($controller->update($id, $title, $description, $image, $buttonText, $buttonLink, $userId)) {
            $_SESSION['success'] = 'Konten home berhasil diperbarui.';
        } else {
    ...
    case 'delete':
        ...
        if ($id <= 0) {
            ...
            if ($controller->delete($id, $userId)) {
                $_SESSION['success'] = 'Konten home berhasil dihapus.';
            } else {
